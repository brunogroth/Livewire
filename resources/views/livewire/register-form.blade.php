<form wire:submit.prevent="submit" class="w-[400px] mx-auto py-16 ">
    ...
    @if ($role === 'vendor')
        <div class="mb-4">
            <input type="text" wire:model.debounce.500ms="company_name" class="w-full border @error('company_name') border-red-500 @enderror"
                   placeholder="Company Name">
            @error('company_name') <span class="text-red-500">{{ $message }}</span> @enderror
        </div>
        <div class="mb-4">
            <input type="text" wire:model.debounce.500ms="cnpj" class="w-full border @error('cnpj') border-red-500 @enderror"
                   placeholder="CNPJ/VAT Number">
